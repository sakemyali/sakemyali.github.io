...
export const projects = [
  ...
  },
  {
    ...
    description: "A staff-support Q&A bot for 42 Tokyo, built in a one-week hackathon. Students /ask in Discord (Japanese or English) and the bot answers from a LightRAG knowledge graph — graph + vector retrieval over 300+ documents (60 scraped intra pages plus ~250 staff Q&A pairs mined from Discord history). A provider-agnostic LLM layer runs either a local model (Qwen2.5-7B via Ollama) or a hosted API (Llama-3.3-70B on Groq, or Gemini), with local sentence-transformer embeddings. When it can't answer, it escalates to staff through paired threads and, on a single ✅, forwards the reply to the asker and live-ingests the new Q+A so the next ask hits it. /search login:<name> returns a bilingual card of where a student is sitting, via the live 42 intra API.",
    tech: ["Python", "LightRAG", "Ollama", "Groq", "42 API"],
    highlights: ["LightRAG retrieval over 300+ docs; local or hosted LLM (Ollama / Groq / Gemini)", "Self-healing: staff escalation → ✅ → live re-ingest", "82% of ~30 surveyed students wanted it adopted"],
  },
  {
    id: 3,
    ...
    description: "A C++17 engine, built for fast option pricing during the IMC Prosperity trading competition, that prices European options under Black-Scholes two ways — closed-form and Monte Carlo — with both analytical and finite-difference Greeks, antithetic variance reduction, and a Newton-with-bisection-fallback implied-volatility solver. A CLI exposes price, a convergence sweep (MC standard error across N from 1K to 10M paths, plain vs antithetic, against the analytical price), and iv. Cross-validated against Hull's Options, Futures, and Other Derivatives (chapters 15, 19, 21), with a Catch2 test suite and an optional native ImGui visualizer.",
